...
@onready var room_scene : PackedScene = load("res://recource/room.tscn")
...
@export var Max_enem_per_room : int
@export var Max_coin_per_room : int
...
func generate() -> void:
	...
	print(stri)
	instantiate_rooms()
	$"../Player".global_position = (first_room_pos * 550) + Vector2(262, 262)
...
func check_room(x : int, y : int, remaining : int, general_direction : Vector2, first_room : bool = false) -> void:
	...
	if north or first_room:
		check_room(x, y+1, max_remaining if first_room else remaining - 1, Vector2.UP if first_room_pos else  general_direction)
	if south or first_room:
		check_room(x, y-1, max_remaining if first_room else remaining - 1, Vector2.DOWN if first_room_pos else  general_direction)
	if east or first_room:
		check_room(x+1, y, max_remaining if first_room else remaining - 1, Vector2.RIGHT if first_room_pos else  general_direction)
	if west or first_room:
		check_room(x-1, y, max_remaining if first_room else remaining - 1, Vector2.LEFT if first_room_pos else  general_direction)
